...
\end{equation}
where $p_n$ denotes the $n$th Legendre polynomial, with a corresponding expansion coefficient $a_n$ obtained from fitting $\log \widehat{T}_0$ up to order $N$. The set of polynomials that one fits to is arbitrary, and our choice of Legendre polynomials is simply one of convenience. \mep{So orthogonality had nothing to do with it? If the polys weren't orthogonal, wouldn't stuff get subtracted off more than once?}
...
\begin{equation}
T(\nu) = \frac{T_{21}}{2} \sqrt{\frac{1+z}{10}} \tanh \left( 1 + \frac{z-z_r}{\Delta z} \right),
\end{equation}
