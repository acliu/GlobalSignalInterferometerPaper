...
\begin{equation}
\mathbf{\hat x} = \M \Q^\dagger \N^{-1} \mathbf{y}
\end{equation}
...
\label{eqn:M}
\end{equation}
which is obtained through a least-squares derivation that minimizes $\chi^2 \equiv (\mathbf{y}-\Q \mathbf{\hat x})^\dagger \N^{-1} (\mathbf{y}-\Q \mathbf{\hat x})$. Note that for this choice of M, the ensemble average of $\mathbf{\hat x}$ is $\mathbf{x}$.
Using this fact, we can directly find the covariance of $\mathbf{\hat x}$ to be 
\begin{equation}
\textrm{Cov}(\mathbf{\hat x}) = [\Q^\dagger \N^{-1} \Q]^{-1}
\end{equation}
\mep{note to self: get a bibtex going and cite Tegmark mapmaking paper here.} Therefore, using equation (\ref{eqn:M}) for o
