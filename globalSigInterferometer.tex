...
noted in \citet{Thyagarajan_et_al2015}. \citet{Mahesh_et_al2014} have also explored the possibility of using a resistive fence to act as a radio-wavelength analog to a beam-splitter, with the fence placed between the two elements of a baseline to increase sensitivity to the monopole. \acl{Added Mahesh citation} \mep{Is this the best place for the Mahesh citation? Unless I am mistaken, their method doesn't use these results, correct? Shouldn't the citation be up in the introduction when we're talking about EDGES and DARE, etc.?}

Returning to our scenario of a simple baseline without a resistive fence, we demonstrate our results numerically with a toy example in Figure \ref{fig:effAperture}. The top panel
