...
\end{equation}
where $\mathscr{L}(\boldsymbol \theta | \xhat)$ is the likelihood function of a set of model parameters $\boldsymbol \theta$ given the measurements $\xhat$, with $\boldsymbol \theta_0$ denoting a set of fiducial values for the parameters. In our case, $\xhat$ should be thought of as a vector containing the final spectrum that one obtains after performing all the analysis steps prior to Section \ref{sec:fitting}. This spectrum, which contains the cosmological signal (parameterized by one of the forms given in Section \ref{sec:SimResults}), foregrounds, and noise, may be written as
\begin{equation}
