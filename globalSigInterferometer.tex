...
\begin{figure}[h]
	...
	\caption{A schematic illustration of the $uv$ plane for an interferometer. The uv plane represents baselines in Fourier space, and an interferometer with baseline $\b$ makes a measurement at the point $\mathbf{u} = \nu\b / c$. Measuring the monopole signal would require making a measurement at the origin of the uv plane, which is probed by a baseline of zero length. This seems impossible without measuring the auto-correlation of an antenna with itself, until one recalls that a real interferometer does not measure an exact point: the antenna's beam has a $uv$ plane footprint that smears the measurement, so that the measurement contains information from nearby baselines. Since a narrower beam corresponds to a larger smear, two narrow-beam antennae placed close together would make a measurement that incorporates information from the monopole, as shown by the green circle in the figure.}
...
\begin{figure}[h]
	...
	\caption{Foreground residuals from Figure \ref{fig:Ncomparison} after a $6$th order logarithmic polynomial subtraction. The uniformly weighted residuals are now seen to be weaker than the those of the inverse foreground weighted analysis. Inverse foreground weighting is thus seen to imprint extra chromaticity in measured spectra, making spectral foreground subtraction more difficult.}
...
\begin{table*}[htbp]
   ...
   \begin{tabular}{@{} llcccccc @{}} % Column formatting, @{} suppresses leading/trailing space
      ...
      \cmidrule(l){2-8}
			 & Interferometer Bias & $-5.72\times 10^{-4} $& -0.00229 & -0.00714 & $1.56\times 10^{-4}$ & 0.00152 & -0.00189 \\
			 & Single Element Bias & $3.70\times 10^{-3}$ & 0.0124 & 0.0409 & $1.07\times 10^{-3}$ & 0.0639 & -0.0237 \\	
      ...
      \cmidrule(l){2-8}
			 & Interferometer Bias & $-3.63\times 10^{-5 }$& $-5.48\times 10{-5}$ & $-4.50\times 10^{-4}$ & $-9.24\times 10^{-5}$ &$ -3.25\times 10^{-4}$ & $8.86\times 10^{-4}$ \\
			 & Single Element Bias & $1.12\times 10^{-3}$ & $-1.25\times 10^{-3}$ &$ 9.95\times 10^{-3}$ & $-6.01\times 10^{-4 }$& $-1.71\times 10^{-2}$ & $6.33\times 10^{-3}$ \\	
      ...
      \bottomrule
   \end{tabular}
   \caption{Table of parameters, biases, and $1\sigma$ error bars for different scenarios and experiments.}
   \label{tab:params}
\end{table*}
...
\begin{figure*}[t]
	...
	\caption{Same as Figure \ref{fig:reionContoursPoly7Poly7}, but for experiments targeting the pre-reionization dip. Primary beams are set to have a FWHM of $38.7^\circ$ at $50\,\textrm{MHz}$. Again, the interferometer performs at roughly the same level as the single-element experiment does, and if 	$N_\textrm{poly}$ is set to 7 for both experiment types, the results are virtually indistinguishable.}
...
Under our insistence that the parameter biases from residual foregrounds be subdominant compared to the parameter errors, we find that it is necessary to set the polynomial order $N_\textrm{poly}$ to $7$ for the case of the interferometer. On the other hand, $N_\textrm{poly}=6$ is sufficient for the single-element experiment. This reflects the increased chromaticity that inevitably comes with interferometry. However, the contrast in spectral smoothness is not dramatic, and from Table \ref{tab:params} we see that these polynomial orders yield final biases that are much smaller for the interferometer than for the single-element experiment. If $N_\textrm{poly}$ is set to $7$ for both experiment types, we find that the errors and biases are virtually indistinguishable.
...
\label{fisher}
In this Appendix, we review the Fisher matrix formalism used in \acl{Cite Gianni} to forecast the error bars and biases in a global signal measurement. We do not claim any originality here, and include a description of the formalism only for completeness and consistency of notation.
