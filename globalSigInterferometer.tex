...
\end{equation}
where $\theta_b$ controls the width of the primary beam. We assume (rather conservatively) that the beam width is proportional to the observation wavelength. To measure the global signal, we compute
\begin{equation}
...
\acl{Make clear that the beam sizes quoted here is at the lowest observation frequency}
...
\begin{figure*}[t]
	...
	\caption{Pairwise parameter contours for interferometer and single element experiments probing the pre-reionization dip. See Figure \ref{fig:reionContoursPoly7Poly7} for the simulation set-up. \acl{Rewrite caption to update}}
